...
\includepdf[pages=1]{contents/stha4.pdf}
...
\end{figure}
Die Kraft $N_1$ (Normalkraft) und $H_1$ (Reibung) wird mit einer Kraft $W_{1}$ ersetzt. Die maximale Haftungswinkel von dieser Kraft kann mit der Haftungskoeﬀizient berechnet werden:
\begin{align*}
  \tan \alpha_{1} &= \mu_1\\
  \alpha_{1} &= 16,7^\circ
\end{align*}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \coordinate (A) at ($(0, \fg) + (118.3:3cm)$);
...
\begin{align*}
  ...
  \alpha_{2} &= 14,04^\circ
\end{align*}
Die vier Kräfte $G_2$, $W_1$, $W_2$, $F_{min}$ sollen im Gleichgewicht sein, also sie bilden ein geschlossenes Dreieck.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
      thick, % Default line thickness
  ...
  \end{tikzpicture}
  \caption{Geschlossenes Dreieck von Kräfte $G_2$, $W_1$, $W_2$, $F_{min}$}
\end{figure}
...
Wenn $F$ ein maximumwert $F_{max}$ beträgt, wirken alle Reibungskräfte maximal gegen dieser Kraft, alle $H_i$ Kräfte werden entgegen gesetzt und gleich Groß sein.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \coordinate (X) at ($($(C)!0.5!(D)$) + (0:5cm)$);
    \pic [draw, angle radius=1cm] {angle = X--Y--Z};
  \end{tikzpicture}
  \caption{Freikörperbild von Körper 1}
\end{figure}
